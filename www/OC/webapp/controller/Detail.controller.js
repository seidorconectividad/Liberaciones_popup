/*global location */
sap.ui.define([
    "com/prodac/liboc/masterdetail/controller/BaseController",
    "sap/ui/model/json/JSONModel",
    "com/prodac/liboc/masterdetail/model/formatter",
    "sap/ui/core/routing/History",
    "com/prodac/liboc/masterdetail/controller/jquery.soap",
    "com/prodac/liboc/masterdetail/controller/xml2json",
    "sap/ui/commons/MessageBox"
], function(BaseController, JSONModel, formatter, History, Soap, xml2json, MessageBox) {
    "use strict";

    return BaseController.extend("com.prodac.liboc.masterdetail.controller.Detail", {
        ...
        onDesaprobarDialog: function(oEvent) {
            ...
            var a = [];
            /*eslint-disable*/
            //console.log("onDesaprobarDialog --- aResult ",o.getProperty("/"));
            $.each(o.getProperty("/"), function(value, index, array) {
                if (index.TABLA === "MOTIVOS_OC") {
                    a.push({ "CODIGO": index.CODIGO, "DESCRIPCION": index.DESCRIPCION });
                }
            });
            console.log("a = ", a);
            mData.items = a;
            ...
            var dialog = new sap.m.Dialog({
                ...
                stretch: false,
                content: [
                    new sap.m.Label({ text: 'Motivo', labelFor: 'submitMotivo' }),
                    new sap.m.Select({
                        id: "submitMotivo",
                        width: '100%',
                        items: {
                ...
                beginButton: new sap.m.Button({
                    ...
                    press: function() {
                        ...
                        var sMotivoCod = sap.ui.getCore().byId('submitMotivo').getSelectedKey();
                        console.log("Motivo : " + sMotivoCod);
                        if (sMotivoCod === "-1") {
                            sMotivoCod = "";
                        }
                        that.onDesaprobar(sMotivoCod);
                        dialog.close();
                    }
                }),
                endButton: new sap.m.Button({
        ...
        onAprobar: function(sComentario) {
            ...
            var wdsl = ['<soap:Envelope xmlns:soap="http://www.w3.org/2003/05/soap-envelope" xmlns:urn="urn:sap-com:document:sap:rfc:functions">',
                ...
                '<I_PURCHASEORDER>' + sNumlib + '</I_PURCHASEORDER>',
                '<I_REL_CODE>' + sCodlib + '</I_REL_CODE>',
                '</urn:Z_MM_SET_ORDCOMPRA_APROB>',
                '</soap:Body>',
                '</soap:Envelope>'
            ];
            ...
            // 'https://destinations-d040c3179.dispatcher.us2.hana.ondemand.com/destinations/SAPDEV_RFC/sap/bc/srt/rfc/sap/zws_mm_set_ordcompra_aprob/130/zservice_z_mm_set_ordcompra_apro/zbinding_z_mm_set_ordcompra_aprob',
            $.soap({
                url: 'https://destinations-d040c3179.dispatcher.us2.hana.ondemand.com/destinations/SAPDEV_RFC/sap/bc/srt/rfc/sap/zws_mm_set_ordcompra_aprob/100/zws_mm_set_ordcompra_aprob/zws_mm_set_ordcompra_aprob',
                data: wdsl.join(''),
                async: true,
                soap12: true,
                context: this,
                ...
                success: function(soapResponse) {
                    ...
                    var json2 = json["#document"]["env:Envelope"]["env:Body"]["n0:Z_MM_SET_ORDCOMPRA_APROBResponse"]["E_GTD_0002"];
                    console.log("onAprobar = ", json2);
                    console.log("Estado = ", json2["item"][1]["TYPE"]);

                    if (json2["item"][1]["TYPE"] == 'S') {
                        console.log("Se aprobó con éxito la liberación OC");
                        ...
                        this.getView().getModel().update(sRequest, oCabecera, {
                            success: function(oData, oResponse) {
                                ...
                                midata.refresh(true);
                                sap.m.MessageToast.show("Se aprobó con éxito la liberación OC", { duration: 6000 });
                            },
        ...
        onDesaprobar: function(sComentario) {
            ...
            // 'https://destinations-d040c3179.dispatcher.us2.hana.ondemand.com/destinations/SAPDEV_RFC/sap/bc/srt/rfc/sap/zws_mm_set_ordcompra_desapr/130/zservice_mm_set_ordcompra_desapr/zbinding_zws_mm_set_ordcompra_desapr',
            $.soap({
                url: 'https://destinations-d040c3179.dispatcher.us2.hana.ondemand.com/destinations/SAPDEV_RFC/sap/bc/srt/rfc/sap/zws_mm_set_ordcompra_desapr/100/zws_mm_set_ordcompra_desapr/zws_mm_set_ordcompra_desapr',
                data: wdsl.join(''),
                async: true,
                soap12: true,
                context: this,
                ...
                success: function(soapResponse) {
                    ...
                    var json2 = json["#document"]["env:Envelope"]["env:Body"]["n0:Z_MM_SET_ORDCOMPRA_DESAPRResponse"]["E_GTD_0002"];
                    console.log("onDesaprobar = ", json2);
                    console.log("Estado = ", json2["item"][1]["TYPE"]);

                    if (json2["item"][1]["TYPE"] == 'S') {
                        console.log("Se desaprobó con éxito la liberación OC");
                        ...
                        this.getView().getModel().update(sRequest, oCabecera, {
                            success: function(oData, oResponse) {
                                ...
                                sap.ui.core.BusyIndicator.hide();
                                midata.refresh(true);
                                sap.m.MessageToast.show("Se desaprobó con éxito la liberación OC", { duration: 6000 });
                            },
                            error: function(oError) {
                                console.log("No se pudo cambiar el estado de la Liberación", oError);
                                sap.ui.core.BusyIndicator.hide();
